...
class AccountMove(models.Model):
    ...
    def _compute_amount(self):
        for move in self:
            ...
            for line in move.line_ids:
                ...
                if move.is_invoice(include_receipts=True):
                    ...
                    if not line.exclude_from_invoice_tab:
                        ...
                        tcs_amount += line.quantity * line.price_unit * line.discount / 100.0
                        ...
                        global_tcs_currency = line.amount_currency
                        total += line.balance
                        total_currency += line.amount_currency
            ...
            if move.move_type == 'entry' or move.is_outbound():
                ...
                sign = -1
            total_tcs_amount = -1 * sign * (global_tcs_currency if len(
                currencies) == 1 else global_discount)
            tcs_amount += total_tcs_amount
            move.total_tcs_amount = total_tcs_amount
            move.tcs_amount = tcs_amount
    ...
    @api.constrains('tcs_percentage', 'invoice_line_ids.discount')
    def validate_only_one_discount(self):
        for rec in self:
            if any(line.discount > 0 for line in rec.invoice_line_ids) and rec.tcs_percentage > 0:
                raise ValidationError("You can only apply one discount at a time. \n Global discount or line discount")

    @api.onchange('tcs_percentage')
    def _onchange_tcs_percentage(self):
        if not self.tcs_percentage:
    ...
    def _recompute_global_discount_lines(self):
        ...
        if self.apply_tcs:
            def _compute_payment_terms(self):
                ...
                discount_balance = sign * (total * (self.tcs_percentage or 0.0) / 100)
                ...
                for date_maturity, balance, amount_currency in to_compute:
                    if existing_global_lines:
                        ...
                        candidate = create_method({
                            ...
                            'account_id': self.partner_id.tcs_account,
            ...
            if existing_global_lines:
                ...
                account = self.env.company.account_tcs
