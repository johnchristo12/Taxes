...
class AccountMove(models.Model):
    ...
    def _compute_amount(self):
        for move in self:
            ...
            for line in move.line_ids:
                ...
                if move.is_invoice(include_receipts=True):
                    ...
                    if not line.exclude_from_invoice_tab:
                        ...
                        tds_amount += line.quantity * line.price_unit * line.discount / 100.0
                        ...
                        global_tds_currency = line.amount_currency
                        total += line.balance
                        total_currency += line.amount_currency
            ...
            if move.move_type == 'entry' or move.is_outbound():
                ...
                sign = -1
            total_tds_amount = -1 * sign * (global_tds_currency if len(
                currencies) == 1 else global_discount)
            tds_amount += total_tds_amount
            move.total_tds_amount = total_tds_amount
            move.tds_amount = tds_amount
    ...
    @api.constrains('tds_percentage', 'invoice_line_ids.discount')
    def validate_only_one_discount(self):
        for rec in self:
            if any(line.discount > 0 for line in rec.invoice_line_ids) and rec.tds_percentage > 0:
                raise ValidationError("You can only apply one discount at a time. \n Global discount or line discount")

    @api.onchange('tds_percentage')
    def _onchange_tds_percentage(self):
        if not self.tds_percentage:
    ...
    def _recompute_global_discount_lines(self):
        ...
        if self.apply_tds:
            def _compute_payment_terms(self):
                ...
                discount_balance = sign * (total * (self.tds_percentage or 0.0) / 100)
                ...
                for date_maturity, balance, amount_currency in to_compute:
                    if existing_global_lines:
                        ...
                        candidate = create_method({
                            ...
                            'account_id': self.partner_id.tds_account,
            ...
            if existing_global_lines:
                ...
                account = self.env.company.account_tds
...
class AccountMoveLine(models.Model):
    ...
    is_global_line = fields.Boolean(string='TDS Amount Line',
